  ...
  return `
    <section class="dash-head">
      <div class="card">
        ...
        <div class="dash-actions"><a class="btn primary" href="#/log?date=${encodeURIComponent(isoDate(new Date()))}">Log today</a><button class="btn" id="copyToday">Copy to Clipboard</button><button class="btn" id="showTodayExport">Show Export Text</button></div><div id="todayExportWrap" class="export-panel" hidden><div class="field"><label>Today export text</label><textarea id="todayExportText" rows="10" readonly></textarea></div></div>
      </div>
